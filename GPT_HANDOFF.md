...
- Reviewed `<external-firework-reference>/Firework.py` as external reference material and documented adaptable preset ideas in `docs/research/external_firework_reference.md`.
...
- `uv` is installed as a user-level pipx app at `<user-local-bin>/uv`.
...
- `T0004.2.8.2` widened only the top-face interior star visibility threshold again so ceiling stars remain visible at near-parallel eye-line-to-ceiling angles. Side-wall star visibility remains unchanged.
...
- `T0005.1` added Pyxel-independent runtime scaffold modules: `runtime/state.py` and `runtime/show_controller.py`. The scaffold represents selected profile, selected firework kind, active scenery, toggles, salvo mode, auto-rotate speed mode, frame count, and seed base, plus pure transition helpers. Preview has not been migrated to use it yet.
- `T0005.2` added Pyxel-independent `runtime/camera_motion.py` and moved settled auto-rotate comfort settings package-side. Preview now uses package-side values for `slow=0.0035`, `normal=0.0065`, `fast=0.0100`, and pitch sway scales `0.25/0.55/0.80`. `X`/`Q` behavior and HUD mode labels remain unchanged.
...
- `T0006.1` extended the BGM into a longer three-channel music-box arrangement. Channels 0-2 are melody, arpeggio accompaniment, and sparse shimmer; channel 3 remains reserved for explosion SFX.
...
- `T0008.0` added `Sphere Bloom` and `Long Willow` as first-class firework variants. `T0008.4` added `Smile` as a shaped firework with two eyes and a smiling mouth arc. Runtime and preview cycle order is Kiku, Sphere Bloom, Smile, Ring, Spiral, Willow, Long Willow, Peony, Multi-ring, Senrin, Halo. Existing Kiku, Peony, and Willow behavior remains available.
- `T0011.2` added `Grand Sphere` as a special official-runtime single-launch firework. It uses a large 3D spherical bloom, longer restrained fade, no delayed mini-burst garnish, and is intentionally excluded from fixed-count salvos, inward pair salvos, and salvo random kind selection. The mobile `MENU` firework selector can choose it for deterministic review and forces mobile `COUNT 1` when it does.
- `T0011.3` increased Grand Sphere particle density and changed Height variation scheduling to deterministic three-level elevated heights. Grand Sphere single launches now go through the scheduled shell path so Height ON can move them while keeping all three levels inside the `iphone16_balanced` observation box.
- `T0011.4` prevents invalid Grand Sphere multi-shot states: if mobile `COUNT` moves away from `1` while Grand Sphere is selected, or if a salvo schedule is requested defensively with Grand Sphere still selected, runtime switches the selected firework back to the normal first-generation default before scheduling.
- `T0011.5` added a mobile `CITY` row companion toggle labeled `HIDE`. When enabled, the runtime hides the camera-nearest vertical edge of the enclosing observation box while leaving CITY geometry, scenery lines, fireworks, and the underlying box geometry unchanged.
...
- `T0008.3.1` tuned only `Long Willow` long-branch trail decay. Long branch particles keep about 56 frames of trail history, draw older rear sections sparsely broken, and now use softened downward velocity/gravity so the trail falls more gently inside the box. Baseline Willow, shell tail, global glitter, palette variants, CITY, stars, UFO, and audio remain unchanged.
...
- `T0010.0` added official-runtime mobile touch controls. Drag/flick on the play field rotates the camera, and the top-right `MENU` opens a Pyxel-rendered panel with scaled larger text, `LAUNCH`, a current-firework selector, `SALVO START`, and `ZOOM+` / `ZOOM-` actions. `T0010.1` made panel checkboxes apply immediately and removed `APPLY`. `T0010.2` added PC mouse-wheel zoom. `T0010.3` shortened the mobile panel and aligned `ZOOM+`, `ZOOM-`, and `CLOSE` on one bottom row. `T0010.4` moved BGM beside audio and added `COUNT` to cycle `1`-`5`/`RND` before pressing `SALVO START`. `T0010.5` made the firework selector wide and labeled with the current kind. `T0010.6` connected `COUNT` to runtime salvo state so selected counts affect `SALVO START` and active repeat salvos. `T0010.7` made mobile `AUTO` use the selected `COUNT` value instead of fixed one-shell launches. `T0010.8` disables Pyxel Web's default virtual gamepad in the public HTML so only the in-app touch panel appears on phones. `T0010.11` keeps mobile `random` firework-type-only and preserves the current `COUNT`, including `COUNT RND`. Mobile `COUNT` now also includes `6` for the inward mirrored pair salvo, and `COUNT RND` may select it. `T0010.12` removed the first-touch BGM retry and Safari WebAudio unlock helper after confirming the iPhone no-audio issue was Silent Mode; no first tap should restart BGM now. Keyboard controls remain available, including `A` / `S` zoom.
- `T0011.0` added keyboard `6` as a persistent mirrored inward pair salvo in the official runtime. It uses 10 horizontal launch positions and fires five simultaneous left/right pairs in order `(0, 9)`, `(1, 8)`, `(2, 7)`, `(3, 6)`, `(4, 5)`. Random firework mode freezes one kind per wave for both launches, while pair seeds differ by 3 so palette variants match without exact seed copies.
