...
- `T0010.0` added official-runtime mobile touch controls. Drag/flick on the play field rotates the camera, and the top-right `MENU` opens a Pyxel-rendered panel with scaled larger text, draft checkboxes, `APPLY`, `LAUNCH`, `NEXT`, `RAND SALVO`, and `ZOOM+` / `ZOOM-` actions. Keyboard controls remain available, including `A` / `S` zoom.
