...
- `T0010.0` added official-runtime mobile touch controls. Drag/flick on the play field rotates the camera, and the top-right `MENU` opens a Pyxel-rendered panel with scaled larger text, `LAUNCH`, `NEXT`, `RAND SALVO`, and `ZOOM+` / `ZOOM-` actions. `T0010.1` made panel checkboxes apply immediately, removed `APPLY`, and added a BGM-only toggle that leaves explosion SFX available when overall audio is on. `T0010.2` added PC mouse-wheel zoom. `T0010.3` shortened the mobile panel and aligned `ZOOM+`, `ZOOM-`, and `CLOSE` on one bottom row so more of the scene remains visible. Keyboard controls remain available, including `A` / `S` zoom.
