...
On PC, use `A` / `S` or the mouse wheel to zoom. Keyboard salvos use `1` for a single-shell loop, `2`-`5` for fixed-count loops, `0` for random-count loops, and `6` for the mirrored inward pair salvo: 10 horizontal launch positions fired as five left/right pairs from the outer edges toward the center. On touch devices, drag/flick the play field to rotate the camera. Tap the top-right `MENU` button to open the mobile control panel; checkbox changes apply immediately behind the panel. The panel uses larger text, is kept short enough to leave part of the scene visible, and includes BGM-only toggle, CITY visibility plus a `HIDE` readability toggle for the nearest vertical edge of the enclosing box, salvo count selection, quick launch, a current-firework selector button, salvo start, and `ZOOM+` / `ZOOM-` buttons. Mobile `COUNT` cycles through `1`-`6` and `RND`; `6` starts the inward mirrored pair salvo, and `RND` can choose it. Mobile `RANDOM` randomizes only firework type and never changes the current `COUNT`; random launch count remains controlled by `COUNT RND`. The public Pyxel Web build disables Pyxel's default virtual gamepad so the in-app touch panel is the mobile control surface.
...
The mobile `MENU` firework selector can also choose Grand Sphere, a denser larger 3D spherical bloom that stays inside the observation box. Selecting Grand Sphere forces mobile `COUNT` back to `1`; moving `COUNT` away from `1` changes the selected firework back to the normal cycle so multi-shot salvos remain valid. Random single-shell launches can also rarely choose Grand Sphere, but multi-shot salvo selections do not. When Height is ON, scheduled burst heights choose one of three deterministic elevated levels.
