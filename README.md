...
On PC, use `A` / `S` or the mouse wheel to zoom. On touch devices, drag/flick the play field to rotate the camera. Tap the top-right `MENU` button to open the mobile control panel; checkbox changes apply immediately behind the panel. The panel uses larger text, is kept short enough to leave part of the scene visible, and includes BGM-only toggle, salvo count selection, quick launch, a current-firework selector button, salvo start, and `ZOOM+` / `ZOOM-` buttons. The public Pyxel Web build disables Pyxel's default virtual gamepad so the in-app touch panel is the mobile control surface.
