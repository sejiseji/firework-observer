...
On PC, use `A` / `S` or the mouse wheel to zoom. On touch devices, drag/flick the play field to rotate the camera. Tap the top-right `MENU` button to open the mobile control panel; checkbox changes apply immediately behind the panel. The panel uses larger text for readability and includes BGM-only toggle, quick launch, next-firework, random-salvo, and `ZOOM+` / `ZOOM-` buttons.
